...
namespace test.ViewModel.Patients.PatientVMs
{
    public class PatientGeneratVM : BaseVM
    {
        ...
        public void DoGen()
        {
            var birthdaystart = new DateTime(1900, 1, 1);
            var random = new Random();
            var allvirus = DC.Set<Virus>().AsNoTracking().ToList();
            var allcity = DC.Set<City>().Include(x => x.Parent).AsNoTracking().ToList().Where(x => x.ParentId != null && x.Parent?.ParentId != null).ToList();

            for (int i = 1; i <= GenerateCount; i++)
            {
                var bdate = birthdaystart.AddDays(random.Next(0, 120 * 365));
                Patient p = new Patient
                {
                    Birthday = bdate,
                    Gender = (Model.GenderEnum)random.Next(0, 2),
                    IdNumber = GenerateCardNumber(bdate),
                    IsValid = true,
                    PatientName = GetName(),
                    Status = (PatientStatusEnum)random.Next(0, 5),
                    LocationId = allcity[random.Next(0, allcity.Count())].ID
                };

                DC.AddEntity(p);
                if (allvirus.Count > 0)
                {
                    PatientVirus pv = new PatientVirus
                    {
                        Patient = p,
                        VirusId = allvirus[random.Next(0, allvirus.Count())].ID
                    };
                    DC.AddEntity(pv);
                }
                //每一千行保存一次
        ...
        private string GetName()
        {
            ...
            string rv = surname[index];
            var names = GenerateChineseWords(random.Next(1, 3));
        ...
        private string GenerateCardNumber(DateTime birthday)
        {
            var rd = new Random();
            string area = provinceid[rd.Next(0, provinceid.Count())];

            //随机码
            string code = rd.Next(1000, 9999).ToString("####");
            //生成完整身份证号
            string codeNumber = area + code + birthday.ToString("yyyyMMdd") + code;
            double sum = 0;
        ...
        private static List<string> surname = new List<string>() {"赵", "钱", "孙", "李", "周", "吴", "郑", "王", "冯", "陈", "楮", "卫", "蒋", "沈", "韩", "杨",
  ...
  "段干", "百里", "东郭", "南门", "呼延", "羊舌", "梁丘", "左丘", "东门", "西门", "南宫"};
        ...
        private static List<string> provinceid = new List<string>()
        {
